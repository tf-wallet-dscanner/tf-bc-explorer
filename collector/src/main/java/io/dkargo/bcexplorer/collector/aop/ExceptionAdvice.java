package io.dkargo.bcexplorer.collector.aop;

import io.dkargo.bcexplorer.core.error.DkargoException;
import io.dkargo.bcexplorer.core.error.ErrorDTO;
...
public class ExceptionAdvice {
    ...
    public ResponseEntity<ErrorDTO> methodArgumentNotValidException(MethodArgumentNotValidException e) {
        String errorMessage = String.format("[%s] 지정된 타입의 값이 아니거나 %s", e.getBindingResult().getFieldError().getField(), e.getBindingResult().getAllErrors().get(0).getDefaultMessage());
